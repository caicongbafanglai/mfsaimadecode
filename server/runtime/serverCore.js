...
function serverTimePayload() {
  ...
  updateUfoScheduler(payload);
  return {
    ...payload,
    ufoEvent: publicUfoEvent(nowMs)
  };
}
...
function maybeStartIslandEvent(time, phase) {
  ...
  const eligiblePlayer = onlinePlayers()
    .find(player => islandTriggerEligible(player.state || {}));
  if (!eligiblePlayer) return false;
  if (phaseLimitReached(phase)) return false;
  const probability = phase === 'night' ? 0.35 : 0.18;
...
function createIslandUfoEvent(time, player) {
  const nowMs = time.serverNowMs;
  const availableIndices = HIDDEN_APRON_UFO_LOCAL_POSITIONS
    .map((_, index) => index)
    .filter(index => !ufoScheduler.departedIslandUfoIndices.has(index));
  if (!availableIndices.length) return null;
  const ufoIndex = availableIndices[Math.floor(Math.random() * availableIndices.length)];
  const spawn = hiddenApronUfoWorldPoint(ufoIndex);
  const targetAltitudeFt = clamp((player.state?.altitude || 7000) + randomBetween(1800, 6200), 4200, 18500);
  const verticalSpeed = randomBetween(80, 180);
  const groundY = 3.1;
  const takeoffMs = clamp(((targetAltitudeFt * FT_TO_M - groundY) / verticalSpeed) * 1000, 3200, 9000);
  const durations = {
    preGlowMs: randomBetween(1000, 3000),
    takeoffMs,
    hoverMs: randomBetween(3000, 8000),
    trackMs: randomBetween(1400, 3200),
    departMs: randomBetween(900, 1700),
    lostMs: 2800
  };
  ...
  return {
    ...
    departureSpeed: randomBetween(800, 2000),
    visibility: 1,
    glowIntensity: 0.18,
...
function publicUfoEvent(nowMs) {
  ...
  if (active.mode === UFO_MODES.PLAYER_SIDE_ENCOUNTER) {
...
function islandTriggerEligible(state) {
  const position = state.position || {};
  ...
  if (altitude < 1000 || altitude > 25000) return false;
  const bearing = bearingDeg(x, z, HIDDEN_AIRPORT.x, HIDDEN_AIRPORT.z);
  return Math.abs(angleDeltaDeg(finiteNumber(state.heading, 0), bearing)) <= 85;
}
